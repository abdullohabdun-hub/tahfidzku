...
async function testLocal() {
  console.log(`\n========================================`);
  console.log(` MEASURING LOCAL POST-INDEX PERFORMANCE`);
  console.log(`========================================`);
  ...
  try {
    ...
    console.log(`Logging in on LOCAL...`);
    ...
    for (const p of pagesToTest) {
      ...
      const shotName = `local_post_${p.name.replace(/\s+/g, '_').toLowerCase()}.png`;
    ...
    console.error(`Error testing LOCAL POST-INDEX:`, err.message);
    await browser.close();
...
async function main() {
  const postResults = await testLocal();
  fs.writeFileSync(path.join(SCRATCH_DIR, 'perf_post_summary.json'), JSON.stringify(postResults, null, 2));
  console.log('\n=== POST-INDEX LOCAL PERF RESULTS ===');
