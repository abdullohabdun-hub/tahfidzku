...
// ============================================================
// FUNGSI TAMBAHAN UNTUK FITUR INPUT SETORAN
// ============================================================

/**
 * Poin 1 — Ziyadah: Ambil daftar surah unik dalam suatu juz.
 * Digunakan untuk mengisi dropdown Surah setelah Juz dipilih.
 */
export function getSurahByJuz(juzNumber: number): Array<{ nomor: number; nama: string; totalAyat: number }> {
  const pagesInJuz = PAGES_DATA.filter(p => p.juz === juzNumber)
  const surahNomorSet = new Set<number>()
  const result: Array<{ nomor: number; nama: string; totalAyat: number }> = []

  for (const page of pagesInJuz) {
    for (const s of page.surahs) {
      if (!surahNomorSet.has(s.nomor)) {
        surahNomorSet.add(s.nomor)
        const meta = SURAH_LIST.find(sl => sl.nomor === s.nomor)
...
/**
 * Poin 1 — Ziyadah: Ambil ayat awal dan akhir dari sebuah surah dalam konteks juz tertentu.
 * Digunakan untuk membatasi range input ayat agar tidak melebihi ayat dalam juz tersebut.
 */
export function getAyatRangeInJuz(juzNumber: number, surahNomor: number): { ayatAwal: number; ayatAkhir: number } {
  const pagesInJuz = PAGES_DATA.filter(p => p.juz === juzNumber)
  let ayatAwal = 999
  let ayatAkhir = 0
...
/**
 * Poin 3 — Rekam Jejak: Hitung ayat selanjutnya setelah setoran terakhir.
 * Contoh: Ahmad setor Al-Mulk 1-5, maka next = { surah: Al-Mulk, ayatAwal: 6 }
 */
export function getNextAyat(
  surahNomor: number,
  ayatAkhirTerakhir: number
): { surahNomor: number; surahNama: string; ayatAwal: number } | null {
  const surahMeta = SURAH_LIST.find(s => s.nomor === surahNomor)
  if (!surahMeta) return null

  // Masih ada sisa ayat di surah yang sama
  if (ayatAkhirTerakhir < surahMeta.totalAyat) {
    return { surahNomor, surahNama: surahMeta.nama, ayatAwal: ayatAkhirTerakhir + 1 }
  }

  // Surah sudah habis, lanjut ke surah berikutnya
  const nextSurah = SURAH_LIST.find(s => s.nomor === surahNomor + 1)
  if (!nextSurah) return null // Sudah surah terakhir (An-Nas)
  return { surahNomor: nextSurah.nomor, surahNama: nextSurah.nama, ayatAwal: 1 }
}

/**
 * Poin 2 — Sabqi/Manzil: Ambil info surah+ayat dari nomor halaman global.
 */
export function getPageInfo(halamanGlobal: number) {
  return PAGES_DATA.find(p => p.halaman === halamanGlobal) ?? null
}

/**
 * Poin 2 — Sabqi/Manzil: Konversi halaman lokal dalam juz ke halaman global.
 */
export function getGlobalPage(juzNumber: number, halamanDalamJuz: number): number | null {
  const juzInfo = JUZ_TABLE.find(j => j.juz === juzNumber)
  if (!juzInfo) return null
  const globalPage = juzInfo.halamanAwal + halamanDalamJuz - 1
  if (globalPage > juzInfo.halamanAkhir) return null
  return globalPage
}

/**
 * Poin 2 — Sabqi/Manzil: Hitung total halaman dalam sebuah juz.
 */
export function getTotalHalamanJuz(juzNumber: number): number {
  const juzInfo = JUZ_TABLE.find(j => j.juz === juzNumber)
  if (!juzInfo) return 20
  return juzInfo.halamanAkhir - juzInfo.halamanAwal + 1
}

/**
 * Poin 4 — Progress: Hitung total hafalan dalam juz (mundur dari 30).
 * Menerima array juz yang sudah selesai, contoh: [30, 29, 28]
 * Mengembalikan total halaman dan juz yang sudah hafal.
 */
export function calculateHafalanProgress(juzSelesai: number[]): {
  totalJuz: number
  totalHalaman: number
  label: string
} {
  const totalJuz = juzSelesai.length
  const totalHalaman = juzSelesai.reduce((acc, juz) => {
    return acc + getTotalHalamanJuz(juz)
  }, 0)

  const label = totalJuz === 0
    ? 'Belum ada hafalan'
    : `${totalJuz} Juz (${totalHalaman} halaman)`

  return { totalJuz, totalHalaman, label }
}
