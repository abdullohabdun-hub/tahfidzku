...
export const surahByNomor: Record<number, { nomor: number, nama: string, totalAyat: number }> = Object.fromEntries(
  SURAH_LIST.map(s => [s.nomor, s])
);
...
/**
 * Menghasilkan prefill form Ziyadah berikutnya dari posisi terakhir santri
 * (field yang disimpan di dokumen santri setelah setiap setoran ziyadah
 * tersimpan, mis. { surahNomor: 67, ayat: 5 }).
 * - posisiTerakhir null/undefined -> santri baru, mulai dari Al-Fatiha ayat 1.
 * - Mengembalikan null bila santri sudah khatam 30 juz (tidak ada "berikutnya").
 */
export function prefillZiyadahBerikutnya(posisiTerakhir) {
  if (!posisiTerakhir) {
    return { surahNomor: 1, namaSurah: surahByNomor[1].nama, ayat: 1 };
  }
  const next = ayatBerikutnya(posisiTerakhir.surahNomor, posisiTerakhir.ayat);
  if (!next) return null;
  return {
    surahNomor: next.surahNomor,
    namaSurah: surahByNomor[next.surahNomor].nama,
    ayat: next.ayat,
  };
}
...
export const getNextAyat = ayatBerikutnya;
export function getPageInfo(juz, hal) { return null; }
