...
import { Home, PlusCircle, History, Award, Clock, LogOut, BookOpen, Calendar, BarChart3, Users } from "lucide-react"
...
export const Route = createFileRoute('/ustadz')({
  loader: async () => {
    try {
      ...
      return { user: null, tenantName: 'TahfidzKu', tenantData: null }
    }
  },
  staleTime: 5 * 60 * 1000,
...
function UstadzLayout() {
  ...
  // Inject CSS variable tema per-lembaga (shared hook — sama dengan admin, santri, wali)
  ...
  const desktopNavItems = [
    { name: "Beranda", path: "/ustadz", icon: <Home className="w-5 h-5" /> },
    { name: "Daftar Santri", path: "/ustadz/santri", icon: <Users className="w-5 h-5" /> },
    { name: "Input", path: "/ustadz/input", icon: <PlusCircle className="w-5 h-5" /> },
    { name: "Absensi", path: "/ustadz/absensi", icon: <Calendar className="w-5 h-5" /> },
    { name: "Riwayat", path: "/ustadz/riwayat", icon: <History className="w-5 h-5" /> },
    { name: "Analitik", path: "/ustadz/analitik", icon: <BarChart3 className="w-5 h-5" /> },
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 pb-20 md:pb-0">
      ...
      <header className="bg-white border-b border-slate-200 px-4 py-2.5 sticky top-0 z-50 shadow-xs">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link to="/ustadz/notifikasi" className="relative p-1.5 text-slate-400 hover:text-primary hover:bg-slate-100 rounded-full transition-colors">
              <Bell className="w-4 h-4" />
              {unreadCount > 0 && (
                <span className="absolute top-1 right-1 w-2 h-2 bg-red-500 border border-white rounded-full"></span>
              )}
            </Link>
            <Link to="/ustadz/profil" className="w-7 h-7 rounded-full bg-primary/15 text-primary flex items-center justify-center font-bold text-xs border border-primary/30 uppercase hover:ring-2 hover:ring-primary transition-all">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex justify-around items-center h-14 z-50 px-2 pb-safe shadow-[0_-4px_6px_-1px_rgb(0,0,0,0.05)]">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`relative flex items-center justify-center p-1.5 rounded-full transition-all duration-300 ${isActive ? "text-primary bg-primary/10 shadow-sm" : "text-slate-500 group-hover:text-primary"}`}>
                <div className={`${isActive ? "scale-110" : "scale-100"} transition-transform duration-300`}>
                  {item.icon}
                </div>
              </div>
              <span className={`text-[10px] transition-all duration-300 ${isActive ? "font-bold text-primary" : "font-medium text-slate-500"}`}>
                {item.name}
              </span>
            </Link>
          )
      ...
      <aside className="hidden md:flex fixed inset-y-0 left-0 w-64 bg-white border-r border-slate-200/60 flex-col z-40 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
        <div className="p-6 flex items-center gap-2 border-b border-slate-100">
          ...
          <span className="font-bold text-xl tracking-tight text-slate-900 truncate" title={tenantName}>{tenantName}</span>
        </div>
            ...
            return (
              <Link
                ...
                className={`
                  ...
                  ${isActive ? "bg-primary/10 text-primary font-bold" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"}
                `}
              >
