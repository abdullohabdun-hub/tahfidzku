...
import { getNotifikasiSantri, tandaiDibacaSantri } from '../../server-fns/notifikasi-santri'
import { useState, useEffect } from 'react'
import { Bell, Check, ChevronRight } from 'lucide-react'
import { format } from 'date-fns'
import { id } from 'date-fns/locale'

export const Route = createFileRoute('/santri/notifikasi')({
  component: NotifikasiPage,
})

function NotifikasiPage() {
  const router = useRouter()
  const fetchNotifikasi = useServerFn(getNotifikasiSantri)
  const markAsRead = useServerFn(tandaiDibacaSantri)
  ...
  const handleItemClick = async (notifId: string, e: React.MouseEvent) => {
     try {
        await markAsRead({ data: { notifikasiId: notifId } })
     } catch(e) {
        console.error(e)
     }
  }
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center pb-4 border-b border-slate-200">
        <div>
          ...
          <p className="text-sm text-slate-500">Pemberitahuan feedback setoran dari Ustadz</p>
        </div>
        <button 
        ...
        {items.map((item, i) => {
          ...
          return (
            <Link 
              to="/santri/riwayat"
              key={notif.id || i}
              onClick={(e) => handleItemClick(notif.id, e)}
